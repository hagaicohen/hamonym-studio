...
**עוד שני פערים שהמשתמש דיווח עליהם באותה בדיקה — טרם טופלו**:
1. **לוגו מקובץ שהועלה** — קבצים שמועלים כרגע משמשים רק כקלט ל-Vision LLM (חילוץ טקסט) ונזרקים אחר כך; אין עדיין flow שממש שומר קובץ שתויג כ"לוגו" ומחבר אותו ל-`campaignLogoUrl`/`entities.logo_url`. דורש בניית העלאת קובץ אמיתית, לא רק מיפוי.
2. **"תיצור לי עמותה"** — בכוונה לא נתמך: הוחלט במפורש ב-Sprint 7 שה-AI Creator עובד רק תחת עמותה קיימת. המשתמש לא ידע את זה — כדאי להבהיר את זה ב-UI במפורש כדי שלא ינחש שזה אמור לעבוד.
