...
**עוד פער שהמשתמש דיווח עליו באותה בדיקה — טרם טופל**:
**לוגו מקובץ שהועלה** — קבצים שמועלים כרגע משמשים רק כקלט ל-Vision LLM (חילוץ טקסט) ונזרקים אחר כך; אין עדיין flow שממש שומר קובץ שתויג כ"לוגו" ומחבר אותו ל-`campaignLogoUrl`/`entities.logo_url`. דורש בניית העלאת קובץ אמיתית, לא רק מיפוי.
