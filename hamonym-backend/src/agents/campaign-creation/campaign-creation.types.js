 ...
 * @property {{emotion: string|null, urgency: string|null, focus: string|null, energy: string|null} | null} designIntent - Creative-direction metadata (2026-07-23) — deliberately NOT colors/templates/CSS (that's a system decision, not the model's), just the emotional intent a future template/palette-mapping layer could act on. See prompt rule 10.
 * @property {{primaryAudience: string|null, storyPattern: string|null, ctaApproach: string|null} | null} contentStrategy - Internal metadata, not shown to the user (2026-07-23) — the model's own account of the creative approach it took, meant for future consumers (analytics, CampaignAdvisor comparing intent vs. actual). See prompt rule 11.
 * @property {{hero: string|null, gallery: string[], hidden: string[], reason: string} | null} galleryCuration - Which uploaded image (by the caller's own "image1"/"image2"... labels) should be the Hero, the display order for the rest, and which to omit entirely (blurry/redundant/irrelevant) — only ever set when images were actually sent to this Brief call (Vision), and every label is re-verified against what was actually sent before being trusted (see brief.builder.js's sanitizeGalleryCuration). null when no images were provided. See prompt rule 12.
 */
