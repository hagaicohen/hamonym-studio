...
- נעילת `Offering.key` אחרי שיש הרשמות תחתיו (ר' `DECISIONS.md`, 2026-07-15) — ההחלטה שה-`key` הופך read-only ברגע שיש `registration_participants` תואמים כבר תועדה, אבל האכיפה עצמה (חסימה ב-Builder) לא מיושמת ב-v1.
