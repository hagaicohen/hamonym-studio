...
## 10. NEW OVERRIDING PRINCIPLE — Billing v1: simplicity and maintainability over unnecessary automation (2026-09-10)

After MASAV onboarding closed, two audits (both read-only, both evidence-first — every claim in them checked against real code or a read-only query against the real production DB, not inferred) were run against the full Billing v1 lifecycle:

1. **Production operational readiness audit** — asked "if Hamonym starts operating real monthly billing today, what can actually break?" Verdict: **YES WITH BLOCKERS** — the mechanical pipeline works and is proven (real statement → real CARD charge → real payment), but nothing produces a tax document for the platform-fee charge, production email has never actually delivered (`EMAIL_ENABLED` has never been `true`), and one real statement (₪7.33, "גדולים מהחיים") was sitting stuck with zero operator-visible next action.
2. **Simplification audit** — asked a different question: not "what's missing," but "how much of what already exists is more sophistication than a live system doing ₪7.61 of total lifetime billing actually needs." Verdict, blunt: the six-step business model (donations → period → calculate → approve → collect → paid/waiting) is real and works, wrapped in a second, larger system whose entire job is watching the first one — 9 background jobs, a `billing_runs` table that has never left `status='draft'`, a parallel findings/alerts system with zero human interactions in its lifetime, and a full MASAV lifecycle that has never processed one real statement.

**The user's decision, now the standing principle for Billing v1 and everything built on top of it going forward:**

> Simplicity and maintainability are first-class requirements — ranked *above* automation, though never above financial correctness. A manual operator step that removes real architectural complexity is a win, not a compromise. Before adding anything to Billing v1, the question is not "would this make the system more complete/sophisticated" but **"does this make the system materially safer, or merely more sophisticated?"** If only the latter, don't build it.

This principle explicitly does **not** license weakening what's already proven. It was corrected once already in its first application (see below) specifically because it was about to freeze two Donation Engine safeguards that had nothing to do with Billing observer complexity.

### 10.1 What was actually changed (the smallest package, twice-corrected)

- **Two Billing Engine observer jobs frozen** (`schedule: null`, still registered, still manually runnable via Admin "Run now", implementation untouched): `billing-approval-consistency` (39 real runs, zero findings ever — duplicates in application SQL what the Approval Engine's own DB triggers already make structurally impossible) and `billing-provisioning-gap` (its gap is now also surfaced directly by `billing-monthly-cycle.job.js`'s own blocked-entity list on every real calculation run).
- **Two Donation Engine jobs deliberately NOT frozen**, after an explicit correction mid-package: `aggregate-consistency` and `stuck-recurring-signups`. Both were initially frozen alongside the Billing pair (they'd been swept up by the simplification audit's broader "financial background jobs" framing), then restored before commit once it was clear they are donor-side safeguards — not Billing v1 observer complexity — each with its own explicit **Approved production schedule (Operational Policy, 2026-08-16)**, `aggregate-consistency`'s specifically written in response to one real silent-drift incident that already happened (Phase 1 cleanup, 2026-08-14). **The principle governs Billing v1's own accumulated complexity — it is not a license to unwind an already-proven Donation Engine safeguard.** See `scripts/test-frozen-jobs-registration.js` for the real-registry proof of exactly this scope (2 frozen, 2 explicitly confirmed still scheduled).
- **Billing Ops' default tab changed from `periods` to `statements`.** The operator's landing view is now debts, not engine periods/runs. Periods & Calculation remains available as a secondary tab — not deleted, not hidden.
- **One new backend-derived, presentation-only field: `next_action`** (`billing-ops.service.js#nextActionLabel`, plus a `חודש` column reusing the existing period-label formatter) on every row of the Statements list. No new DB state, no new table, no new job — it's a pure function over facts that were already authoritative (`evaluateCollectionReadiness`, itself a thin wrapper over `routing.js`; `status`; `latest_attempt_status`). Verified against real production data the same day it shipped: the previously-stuck ₪7.33 statement now shows **"חסר כרטיס אשראי"** directly in the list, instead of requiring the operator to open a drawer and infer the block from a generic "דורש טיפול".
- **The billing-setup-notification automatic-retry implementation from earlier the same day was rolled back before ever being committed.** Kept: `email.service.js`'s `dispatch()` returning `{status}` (additive, zero behavior change for existing `queue()` callers), and migration 065's `delivered` column (already applied to production) — used only to honestly record whether one attempt actually went out, never to gate an automatic retry. `notifyBillingSetupRequired` is back to its original shape: `INSERT ... ON CONFLICT DO NOTHING`, attempt once, no row-locking, no reuse branch, no retry lifecycle. The reasoning that killed the retry design: the channel it would have retried has **never once delivered in production** (`email_logs`: 0 `sent` rows, ever) — automatic retry of a channel that doesn't exist yet is complexity with no payoff.
- **The unsupported registration promise** ("חשבונית וקבלה יישלחו למייל הארגון", about the platform fee specifically, `step-billing-method.component.html`) replaced with neutral wording that doesn't promise a document-issuance capability the system doesn't have.
- **Explicitly not done, by decision, not oversight:** the routing-rule consolidation (4 real duplicate implementations of the CARD/MASAV threshold+authorization decision were found and precisely located, but consolidating them means changing three real, working SQL query shapes — deliberately left as documented technical debt rather than refactored for architectural cleanliness alone); `billing_runs` cleanup; findings-system cleanup; CardCom accounting-document integration (though the research finding that CardCom's `Documents/CreateDocument` endpoint can issue a proper tax document standalone, independent of any charge, verified directly against CardCom's real `swagger/v11/swagger.json`, is preserved as a real future option — not built); production email activation; any further MASAV development.

### 10.2 The Billing Ops operator model (now the standing product definition, not just this package's UI)

```
Statement → amount → collection route → current status → next action
```
