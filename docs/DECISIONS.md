...
**Decision (טרם מומש — תיעוד רעיון בלבד):** להוסיף בעתיד מצב תצוגה נוסף לקמפיין — "דף תרומה פשוט" (Simple Donation Page) — לצד עמוד הקמפיין המלא הקיים. לא סוג קמפיין חדש ולא מנוע תרומות חדש: אותו Campaign, אותו Donation Engine/CardCom/recurring/קבלות/Billing, רק מצג ציבורי אחר — מיועד לתורם שכבר החליט לתרום (סכום ← פרטים ← תשלום, בלי סיפור/עדכונים/תשורות/פיד תורמים). מאפשר גם קישור "תרמו לנו" ישיר וקבוע להפצה (WhatsApp/אתר/QR וכו') מבלי לבנות קמפיין מבוסס-סיפור. השראה: צילום מסך של דף תרומה חיצוני עם חיכוך מינימלי (עדיין לא נוסף לריפו — ר' §5 במסמך המצורף).
