...
**⚠️ ההמלצה הזו הוחלפה בפועל — ר' חלק י"א בסוף המסמך (2026-08-18). In-process scheduler הוכח לא-אמין ב-production (Render מרדים Web Service חינמי אחרי 15 דק' ללא HTTP traffic, וה-timer בתוך אותו process פשוט לא רץ בזמן השינה). הוחלף ב-Render Cron Job נפרד. הטיעון למטה נשאר כאן כתיעוד ההיסטוריה/הנימוק המקורי, לא כמצב נוכחי.**

**המלצה מקורית (הוחלפה), מנומקת לא כי היא נוחה:**
...
**✅ Scheduler מומש (2026-08-16) — קוד קיים, `ENABLE_JOB_SCHEDULER` ברירת מחדל `false`, לא deploy.** `src/jobs/scheduler.js` חדש + `job-runner.js`'s `get(name)` + חיבור ל-`server.js` (עולה אחרי שהשרת מאזין, בלי `runOnInit`, `SIGTERM` עוצר scheduling חדש בלי לחכות לריצה בעיצומה). כל tick עובר דרך `jobRunner.run()` הקיים — אין locking/error-handling חדש, רק timer דק שקורא לתשתית שכבר נבדקה.

**עודכן 2026-08-16 (המשך אותו יום) — `node-cron` שודרג ל-4.6.0, `package.json` קיבל `engines.node ">=20"`.** ההחלטה המקורית (להישאר על 3.0.3) נבדקה מחדש: **בדיקה סטטית** (לא הרצה חיה — `nvm-windows` המותקן מקומית לא הגיב לפקודות, כנראה דורש הרשאות admin שלא היו זמינות בסביבה הזו) הראתה שהתלות הקומפילטיבית היחידה בפרויקט, `bcrypt`, מצהירה `engines: {"node": ">=18"}` בלי תקרה עליונה — לא חוסמת Node 20. אין שום תלות אחרת עם binding native. API של `node-cron@4.6.0` (`schedule()`+`.stop()`) אומת תואם לקוד הקיים ב-`scheduler.js` בלי שינוי לוגיקה. **⚠️ מה שלא אומת ולא ניתן לאמת מה-repo:** גרסת Node שבפועל מוגדרת ל-Render production — **אין** `.nvmrc`/`render.yaml`/`Dockerfile`/`engines` (עד עכשיו) בריפו, כל ההגדרה דרך Render Dashboard בלבד, לא נגישה מכאן. הוספת `engines.node` היא ה-signal ברמת ה-repo היחיד שקיים כרגע — Render בדרך כלל קוראת אותו, אבל **זה לא אומת בפועל בלי deploy אמיתי**, שלא בוצע היום.
...
## חלק י"א — In-process scheduler הוחלף ב-Render Cron Job (2026-08-17/18)

**מה נמצא:** `ENABLE_JOB_SCHEDULER=true` הופעל בפרודקשן (17/08), אומת מיידית שעובד (שלושה ticks טבעיים תקינים, `triggered_by='scheduler'`). בבדיקת בוקר למחרת (18/08) התגלה פער אמיתי: **כ-16 שעות ו-15 דקות בלי אף `job_runs` אחד** — למרות שהריצה האחרונה שנרשמה הייתה `success` (אין שום `failed`). נבדק ישירות (לא הונח): גם אפס פעילות אחרת מונעת-HTTP (webhooks/donations) באותו חלון — לא רק ה-cron שתק, שום דבר לא הגיע לשרת. שולל תקלת קוד טהורה (אותו קוד עבד מצוין לפני ואחרי הפער, בלי deploy ביניים).

**Root cause:** Render מרדים Web Service חינמי אחרי 15 דקות ללא HTTP/WebSocket traffic נכנס (מתועד רשמית ב-Render docs). בזמן השינה שום in-process timer לא רץ — כולל `node-cron`. `runOnInit=false` (החלטה מכוונת מקורית, ר' `scheduler.js`) אומר שאין גם catch-up אוטומטי כשה-process מתעורר.

**החלטה:** Render Cron Job **נפרד** מה-Web Service — תהליך משלו, בטריגר עצמאי של Render, לא תלוי בשינה/ער של ה-Web Service. **אחד, לא ארבעה** (Render מחייב מינימום לכל Cron Job service בנפרד) — Cron יחיד מתעורר כל 15 דקות (התדירות הצפופה מבין הארבעה) ובכל invocation בודק בעצמו אילו מארבעת ה-jobs "הגיע התור" שלהם.

**עיצוב קריטי — לא "זמן שחלף מאז הצלחה אחרונה":** ניסיון ראשון להגדיר "due" כ-`now - lastSuccess >= minGap` נדחה במפורש (ביקורת היוזר, 2026-08-18) — זה סוחף את שעת-ההרצה בפועל של job "פעם ביום ב-03:00" קדימה בכל פעם שהוא מתעכב, במקום לשמר את המשמעות העסקית של "03:00". **התיקון:** `src/jobs/schedule-window.js` — job "due" אם **החלון המתוזמן האחרון שלו** (ההתאמה האחרונה של ה-cron string שלו עד/כולל עכשיו) **אין לו ריצה מוצלחת מאז שהחלון הזה התחיל**. זה משמר סמנטיקת cron אמיתית: job יומי ב-03:00 שפוספס עדיין "מחכה לחלון 03:00 של היום" גם ב-08:00 (catch-up אמיתי), ואחרי שרץ פעם אחת בשבילו — לא רץ שוב עד 03:00 למחרת (אין double-run). מומש בלי תלות בספריית cron-parser חדשה — 4 התבניות הקיימות (`*/15`, `0 * * * *` ×2, `0 3 * * *`) לא דורשות את זה.

**נבדק (regression, לא רק unit) לפני push:** תרחישי 15-דק'/שעתי/יומי due-not-due, catch-up אחרי פספוס, אין double-run לאותו חלון (DB אמיתי, job סינתטי), advisory lock עדיין מגן על ריצה מקבילה (`skipped_locked`), staleness alert. **באג אמיתי נתפס באמצע:** הניסוח הראשוני ל-staleness (`now - windowStart > interval×2`) היה בלתי-אפשרי מבנית — `windowStart` תמיד קרוב ל-`now` בהגדרה (זה "החלון האחרון שכבר חלף"), אז המרחק ממנו לעולם לא יכול לעבור אינטרוול שלם. תוקן: staleness נמדד מ**זמן ריצה מוצלחת אחרונה בפועל** (unbounded), לא יחסית לחלון הנוכחי — עדיין בגודל טולרנס (×2) שנגזר מאותו schedule.

**`job_stale` alert חדש:** `cardcom-ops.controller.js`'s `getHealth`/`computeAlerts` בדק עד כה רק `status==='failed'` — עיוור לחלוטין לתרחיש "job פשוט לא רץ בזמן" (בדיוק הפער שגילינו — הריצה האחרונה שכן נרשמה הייתה `success`, אז לא היה שום alert לאורך כל 16 השעות). נוסף `computeStaleAlerts` שמריץ `checkStaleness` על כל job רשום, מציג "X לא רץ בהצלחה Y דקות" כש-Y עובר פי 2 מהאינטרוול הצפוי שלו. **אומת (read-only) נגד המצב האמיתי:** זיהה נכון את שתי הבעיות שכבר היו ידועות (`webhook-recovery`/`aggregate-consistency` stale) והשאיר את שתי ה-jobs השעתיות (בתוך הטולרנס) ללא alert שגוי.

**קבצים:** `src/jobs/schedule-window.js` (חדש), `src/jobs/cron-entry.js` (חדש — entry point ל-Render Cron), `cardcom-ops.controller.js` (`job_stale`). Committed+pushed: `307e433` (התכונה), מוזג ל-`origin/main` (`3833a9d`).

**✅ Rollout הושלם (2026-08-18):** Render Cron Job הוגדר (`node src/jobs/cron-entry.js`, `*/15 * * * *`), env vars מוגבלים לנדרש בפועל (לא כל סביבת ה-Web Service — `DB_*` + `HAMONYM_CARDCOM_*` בלבד, לפי audit נפרד של שרשרת ה-`require` האמיתית). `ENABLE_JOB_SCHEDULER` על ה-Web Service כובה לפני ההפעלה, כדי לא להריץ שני מנגנוני טריגר מקבילים (advisory lock היה מגן גם ככה, אבל לא הוסתמך על זה בכוונה). **אומת בפרודקשן אמיתי:** ריצה שנייה של ה-Cron הראתה בדיוק את ההתנהגות המצופה — `webhook-recovery` קיבל חלון חדש (17:15), רץ, `success`; שלושת ה-jobs האחרים זיהו שהחלון שלהם כבר בוצע ודילגו (`skipping`) — אין double-run, אין failures, אין קריאות מיותרות ל-CardCom. `node-cron`/`scheduler.js` הישן נשאר בקוד (env-gated, כבוי) — לא הוסר, לא דחוף.
